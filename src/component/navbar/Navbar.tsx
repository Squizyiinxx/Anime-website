  ...
  return (
    <>
      <nav className={`flex w-full justify-between items-center md:py-3 transition-all duration-300 px-[100px] py-0 lg:py-5 fixed top-0 z-[999] ${isScrolled && !sideBar && "bg-white bg-opacity-25 backdrop-blur-lg"}`}>
        ...
        <div
          className={`bg-slate-100 p-5 bg-white bg-opacity-25 h-screen absolute lg:relative w-full lg:w-[80%] flex justify-end lg:justify-around flex-col-reverse lg:flex-row items-center lg:h-0 lg:left-0 lg:bg-transparent 
        top-0 ${sideBar ? "right-0 backdrop-blur-lg" : "-left-full"}
        `}
        >
          <Navlink />
          <Search />
          <div
            ...
            className={`lg:hidden absolute top-10 left-[5%] flex-col w-max flex gap-2 cursor-pointer group transition-all p-3 duration-300 ${!sideBar ? "rotate-180 -translate-x-10" : "rotate-0 translate-x-0"}`}
          >
